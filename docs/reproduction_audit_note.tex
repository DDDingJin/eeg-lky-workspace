...
\item subject conditioning that is present in one model but absent in another
\end{itemize}
...
\item \texttt{HappyQuokka} now has both a \texttt{g\_con=True} subject-conditioned version and a matched \texttt{g\_con=False} non-conditioned version
\end{itemize}

\paragraph{What \texttt{g\_con} means.}
In the upstream \texttt{HappyQuokka} implementation, \texttt{g\_con} enables a global conditioner based on subject identity.
\begin{itemize}
\item \texttt{g\_con=True}: the network receives EEG input plus an explicit subject identity embedding
\item \texttt{g\_con=False}: the network receives EEG input only
\end{itemize}
This distinction matters methodologically:
\begin{itemize}
\item \texttt{g\_con=True} is a stronger within-subject configuration
\item \texttt{g\_con=False} is the fairer row for comparison against baselines that do not receive subject identity
\end{itemize}
...
HappyQuokka adapter & src/repro/happyquokka\_reference.py & official PyTorch HappyQuokka wrapper on unified reference splits, with explicit \texttt{g\_con} control \\
HappyQuokka summary builder & scripts/summarize\_happyquokka\_runs.py & produces paired conditioned/non-conditioned tables and training-curve figures \\
\bottomrule
\end{longtable}
...
conditioned\_deep & HappyQuokka \texttt{g\_con=True} & official PyTorch reference wrapper & 640 x 64 window & 640 x 1 envelope sequence & window 640, random train crops & Adam lr=5e-4 + StepLR & best validation Pearson checkpoint within fixed 100-epoch budget & pooled multi-subject train/val, subject-wise test report & subject identity embedding is provided as a global conditioner \\
conditioned\_deep & HappyQuokka \texttt{g\_con=False} & official PyTorch reference wrapper & 640 x 64 window & 640 x 1 envelope sequence & window 640, random train crops & Adam lr=5e-4 + StepLR & best validation Pearson checkpoint within fixed 100-epoch budget & pooled multi-subject train/val, subject-wise test report & same network family, but without subject identity input \\
\bottomrule
\end{longtable}
...
weissbart\_tf64 & HappyQuokka \texttt{g\_con=True} & conditioned\_reference & 100.0 & 100.0 & 90.0 & 1 \\
weissbart\_tf64 & HappyQuokka \texttt{g\_con=False} & nonconditioned\_reference & 100.0 & 100.0 & 99.0 & 1 \\
etard\_tf64 & HappyQuokka \texttt{g\_con=True} & conditioned\_reference & 100.0 & 100.0 & 94.0 & 1 \\
etard\_tf64 & HappyQuokka \texttt{g\_con=False} & nonconditioned\_reference & 100.0 & 100.0 & 86.0 & 1 \\
\bottomrule
\end{longtable}
...
\item \texttt{experiments/summary\_figures/happyquokka\_conditioning\_summary.csv}
\end{itemize}
...
\item \texttt{HappyQuokka} becomes materially stronger when subject conditioning is enabled
\end{itemize}
...
\item whether subject identity is available to one model family but not to the others
...
\item \texttt{experiments/summary\_figures/reproduction\_audit\_run\_metadata.csv}
\item \texttt{experiments/summary\_figures/happyquokka\_conditioning\_summary.csv}
\item \texttt{experiments/summary\_figures/happyquokka\_conditioning\_overview.png}
\item \texttt{experiments/summary\_figures/happyquokka\_training\_curves.png}
\item \texttt{experiments/summary\_figures/unified\_reference\_main\_summary.csv}
...
\item \texttt{src/repro/happyquokka\_reference.py}
\end{itemize}
