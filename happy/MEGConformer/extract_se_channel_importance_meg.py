"""Extract SE-channel attention weights and back-project to EEG channel importance (MEGConformer)."""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Extract SE channel attention importance (MEGConformer 64ch)')
    ...
    parser.add_argument('--sensor_pos_file', type=str,
                        default='/RAID5/projects/likeyang/happy/MEGConformer/data/sensors_mecg64.mat',
                        help='Path to sensors_mecg64.mat with real 3D electrode positions.')
...
def load_sensor_positions(mat_path: str) -> np.ndarray:
    """Load 2D projected electrode positions from sensors_mecg64.mat.

    Returns pos2d: [64, 2] array normalized to unit circle (MNE convention).
    """
    mat = scipy.io.loadmat(mat_path)
    pos3d = mat['pos']  # [64, 3]
    pos2d = pos3d[:, :2].astype(float)
    # 中心化并归一化到单位圆，MNE plot_topomap 期望坐标在 [-1, 1] 附近
    pos2d -= pos2d.mean(axis=0)
    scale = np.max(np.linalg.norm(pos2d, axis=1))
    if scale > 1e-8:
        pos2d /= scale
    return pos2d
...
def visualize_outputs(visual_dir: Path,
    ...
    plt.xticks(range(len(BIOSEMI_64_CHANNELS)), BIOSEMI_64_CHANNELS, rotation=90, fontsize=11, fontweight='bold')
    ...
    # Top-10 channels horizontal bar
    ...
    if per_subject_avg:
        ...
        plt.xticks(range(len(BIOSEMI_64_CHANNELS)), BIOSEMI_64_CHANNELS, rotation=90, fontsize=11, fontweight='bold')
        plt.yticks(fontsize=11, fontweight='bold')
        plt.ylabel('Std Dev', fontsize=14, fontweight='bold')
    ...
    # Brain topography using real MEG sensor positions from .mat file
    try:
        import mne
        pos2d = load_sensor_positions(sensor_pos_file)  # [64, 2]

        topo_values = np.asarray(global_avg, dtype=float)
        topo_values = topo_values - topo_values.min()
        vmax = topo_values.max()
        if vmax < 1e-8:
            vmax = 1.0
        topo_values = topo_values / vmax
        topo_values = topo_values ** 1  # gamma correction to highlight highs

        fig, ax = plt.subplots(figsize=(5, 4))
        im, _ = mne.viz.plot_topomap(topo_values, pos2d, axes=ax, show=False,
                                     cmap='coolwarm', contours=0,
                                     extrapolate='head', sphere=(0, 0, 0, 1.0))
        cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
        cbar.set_label('Relative importance', rotation=270, labelpad=18)
        fig.tight_layout()
        fig.savefig(visual_dir / 'brain_topography.png', dpi=200)
        plt.close(fig)
        print(f"  脑地形图已保存（使用 sensors_mecg64.mat 真实坐标）")
...
def main():
    ...
    weight_matrix = model.input_proj.weight.detach().cpu()  # [d_model, 64]
