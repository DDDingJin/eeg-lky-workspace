...
def statistical_comparison(all_data, output_dir='comparison_results'):
    ...
    conformer_subject_map = {s['subject_id']: s['pearson'] for s in conformer_data['subject_data']}
    ...
    for data in all_data:
        ...
        baseline_subject_map = {s['subject_id']: s['pearson'] for s in data['subject_data']}
        common_ids = sorted(set(conformer_subject_map) & set(baseline_subject_map))
        conformer_scores = np.array([conformer_subject_map[i] for i in common_ids])
        baseline_scores = np.array([baseline_subject_map[i] for i in common_ids])
...
def plot_comparison(all_data, output_dir='comparison_results'):
    ...
    if conformer_idx is not None:
        conformer_data = all_data[conformer_idx]
        conformer_subject_map = {s['subject_id']: s['pearson'] for s in conformer_data['subject_data']}
        for idx, data in enumerate(all_data):
            if idx == conformer_idx:
                ...
                if len(common_ids) < 2:
                    ...
                    continue
                a = np.array([conformer_subject_map[i] for i in common_ids])
                b = np.array([baseline_subject_map[i] for i in common_ids])
