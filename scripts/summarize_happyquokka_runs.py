...
RUNS = [
    ("weissbart_tf64", "HappyQuokka g_con=True", ROOT / "experiments" / "happyquokka_reference" / "weissbart_tf64_e100_gcon" / "summary.json"),
    ("weissbart_tf64", "HappyQuokka g_con=False", ROOT / "experiments" / "happyquokka_reference" / "weissbart_tf64_e100_nogcon" / "summary.json"),
    ("etard_tf64", "HappyQuokka g_con=True", ROOT / "experiments" / "happyquokka_reference" / "etard_tf64_e100_gcon" / "summary.json"),
    ("etard_tf64", "HappyQuokka g_con=False", ROOT / "experiments" / "happyquokka_reference" / "etard_tf64_e100_nogcon" / "summary.json"),
]
...
def plot_conditioning_summary(df: pd.DataFrame) -> None:
    ...
    condition_order = ["HappyQuokka g_con=False", "HappyQuokka g_con=True"]
    color_map = {
        "HappyQuokka g_con=False": "#537A8A",
        "HappyQuokka g_con=True": "#C14C64",
    }
    ...
    ax.set_title("HappyQuokka Conditioning Comparison")
    ax.set_ylim(0.0, float(plot_df["mean_test_metric"].max() + plot_df["std_test_metric"].max()) + 0.08)
    ax.legend(loc="upper left")
    fig.tight_layout()
    fig.savefig(OUT_DIR / "happyquokka_conditioning_overview.png")
    plt.close(fig)


def plot_training_curves(curve_df: pd.DataFrame) -> None:
    dataset_order = ["weissbart_tf64", "etard_tf64"]
    label_order = ["HappyQuokka g_con=False", "HappyQuokka g_con=True"]
    color_map = {
        "HappyQuokka g_con=False": "#537A8A",
        "HappyQuokka g_con=True": "#C14C64",
    }
    ...
    fig.tight_layout()
    fig.savefig(OUT_DIR / "happyquokka_training_curves.png")
    plt.close(fig)
...
def main() -> int:
    ...
    summary_df.to_csv(OUT_DIR / "happyquokka_conditioning_summary.csv", index=False)
    curve_df.to_csv(OUT_DIR / "happyquokka_training_curves.csv", index=False)
